...
public class KafkaStreamsLiveTest {
  ...
  public void shouldTestKafkaTableLatestWord() throws InterruptedException {
    ...
    textLinesTable
        .toStream()
        .foreach((word, count) -> System.out.println("Latest word: " + word + " -> " + count));
  ...
  public void shouldTestWordCountKafkaStreams() throws InterruptedException {
    ...
    KStream<String, String> textLines =
        builder.stream(wordCountTopic, Consumed.with(Serdes.String(), Serdes.String()));
  ...
  public void shouldTestAggregationStatefulTransformations() throws InterruptedException {
    ...
    final StreamsBuilder builder = new StreamsBuilder();
    final KStream<byte[], String> input =
        builder.stream(aggregationTopic, Consumed.with(Serdes.ByteArray(), Serdes.String()));
    final KTable<String, Long> aggregated =
        input
            .groupBy(
                (key, value) ->
  ...
  public void shouldTestWindowingJoinStatefulTransformations() throws InterruptedException {
    ...
    KStream<String, String> leftRightSource =
        leftSource
            .outerJoin(
                rightSource,
  ...
  public void shouldTestWordCountWithInteractiveQueries() throws InterruptedException {
    ...
    final KGroupedStream<String, String> groupedByWord =
        textLines
            .flatMapValues(value -> Arrays.asList(value.toLowerCase().split("\\W+")))
            .groupBy((key, word) -> word, Grouped.with(stringSerde, stringSerde));

    groupedByWord.count(
        Materialized.<String, Long, KeyValueStore<Bytes, byte[]>>as("WordCountsStore")
