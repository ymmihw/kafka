...
class KafkaProducerUnitTest {
  ...
  private void buildMockProducer(boolean autoComplete) {
    this.mockProducer =
        new MockProducer<>(autoComplete, new StringSerializer(), new StringSerializer());
  }
  ...
  void givenKeyValue_whenSend_thenVerifyHistory() throws ExecutionException, InterruptedException {
    ...
    Future<RecordMetadata> recordMetadataFuture =
        kafkaProducer.send("data", "{\"site\" : \"baeldung\"}");
  ...
  void givenKeyValue_whenSendWithPartitioning_thenVerifyPartitionNumber()
    ...
    this.mockProducer =
        new MockProducer<>(
            cluster,
            true,
            new EvenOddPartitioner(),
            new StringSerializer(),
            new StringSerializer());
    // when
    kafkaProducer = new KafkaProducer(mockProducer);
    Future<RecordMetadata> recordMetadataFuture =
        kafkaProducer.send("partition", "{\"site\" : \"baeldung\"}");
